...
export default defineConfig(({ mode }) => {
  ...
  const target = env.mira_API_URL ?? "http://127.0.0.1:8765";
  const hmrPath = "/__mira_vite_hmr";
  ...
  return {
    ...
    build: {
      outDir: path.resolve(__dirname, "../mira/web/dist"),
      emptyOutDir: true,
      sourcemap: false,
      rollupOptions: {
        output: {
    ...
    server: {
      ...
      // Keep Vite's HMR socket on a dedicated path. mira's app WebSocket is
