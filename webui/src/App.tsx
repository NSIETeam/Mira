...
function RuntimeSnapshotPanel({
  ...
  const schedulerSummary = scheduler && "running" in scheduler
    ? {
        ...
        sessionRunningLimit: scheduler.session_running_limit ?? 0,
  ...
  const latestActivityFreshness = memory?.subagent_memory?.latest_activity_freshness ?? "idle";
        ...
        <div className="mt-3 grid gap-2 text-xs text-foreground">
          ...
          {schedulerSummary ? (
            <div className="flex flex-wrap items-center gap-3 text-muted-foreground">
              ...
              {schedulerSummary.sessionRunningLimit ? (
                <span>running/session {schedulerSummary.sessionRunningLimit}</span>
              ) : null}
          ...
          {recentSubagentEntries.length ? (
            <div className="flex flex-col gap-1 text-[11px] text-muted-foreground">
              <div className="flex flex-wrap items-center gap-3">
                ...
                {latestSubagentActivityAt ? <span>freshness {latestActivityFreshness}</span> : null}
