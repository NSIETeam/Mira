...
const SLASH_RECENTS_STORAGE_KEY = "mira.webui.slashCommandRecents";
const SLASH_RECENTS_LIMIT = 5;
const QUEUED_PROMPTS_STORAGE_PREFIX = "mira.webui.composerQueuedGuidance.v1:";
  ...
  return (
    ...
    >
      {goalPanelOpen && canExpandGoal && markdownBody ? (
        <div
          ref={panelRef}
          id="mira-goal-panel-root"
          role="dialog"
          aria-modal="false"
          aria-labelledby="mira-goal-panel-title"
          tabIndex={-1}
          className={cn(
          ...
          >
            ...
            {canExpandGoal ? (
              <button
                ...
                aria-controls={goalPanelOpen ? "mira-goal-panel-root" : undefined}
...
export function ThreadComposer({
  ...
  const inputTextClasses = cn(
    ...
    isHero
      ? cn(
          "min-h-[72px] px-4 text-[16px] leading-6 sm:px-5",
          relaxedHeroInput ? "pb-2 pt-4" : "pb-2 pt-3.5",
        )
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          "thread-composer-surface group/composer relative mx-auto flex w-full min-w-0 flex-col overflow-visible border border-slate-300/80 bg-white shadow-[0_18px_56px_rgba(15,23,42,0.15)] transition-all duration-200 focus-within:border-slate-950 focus-within:shadow-[0_22px_72px_rgba(15,23,42,0.20)] dark:border-white/15 dark:bg-card",
          isHero
            ? "max-w-[48rem] rounded-[24px] ring-1 ring-slate-950/5"
      ...
      >
        ...
        {isHero ? (
          <div className="px-4 pt-3 text-left text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400 sm:px-5">
