...
describe("ThreadComposer", () => {
  ...
  it("renders a readonly hero model composer when provided", () => {
    ...
    expect(input.className).toContain("pt-3");
  ...
  it("keeps project selection as a compact composer dropdown", async () => {
    const onWorkspaceScopeChange = vi.fn();
    const defaultScope = {
      project_path: "/Users/test/.mira/workspace",
  ...
  it("prioritizes stop as an immediate slash action while streaming", () => {
    ...
    expect(window.localStorage.getItem("mira.webui.slashCommandRecents")).toBeNull();
  });

  it("orders recent slash commands first for the blank slash menu", () => {
    window.localStorage.setItem("mira.webui.slashCommandRecents", JSON.stringify(["/history"]));
    render(
      <ThreadComposer
        onSend={vi.fn()}
        placeholder="Type your message..."
