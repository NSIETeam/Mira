...
/**
 * Minimal fake WebSocket implementing the subset miraClient touches.
...
afterEach(() => {
  Reflect.deleteProperty(window, "miraHost");
  vi.useRealTimers();
});
...
describe("miraClient", () => {
  ...
  it("can swap the socket factory when the runtime URL changes", () => {
    ...
    client.close();
    client.updateUrl("mira-host://engine/", hostFactory);
    client.connect();

    expect(hostFactory).toHaveBeenCalledWith("mira-host://engine/");
    expect(lastSocket().url).toBe("host:mira-host://engine/");
  });
  ...
  it("uses the host socket bridge for native host URLs", async () => {
    ...
    Object.defineProperty(window, "miraHost", {
      configurable: true,
      value: {
        openSocket,
    ...
    expect(openSocket).toHaveBeenCalledWith("mira-host://engine/");
