...
describe("ExecutionList", () => {
  ...
  it("groups WebUI executions by workspace project while preserving in-project sorting and activity", () => {
    const sessions = [
      ...
      session({
        ...
        workspaceScope: {
          project_path: "/Users/me/mira-bench",
          project_name: "mira-bench",
          access_mode: "full",
        },
      }),
    ];
    ...
    const miraSection = screen.getByRole("region", { name: "mira" });
    const miraText = miraSection.textContent ?? "";

    expect(screen.getByRole("region", { name: "mira-bench" })).toBeInTheDocument();
    expect(within(miraSection).getByText("Alpha task")).toBeInTheDocument();
    expect(within(miraSection).getByText("Zeta task")).toBeInTheDocument();
    expect(miraText.indexOf("Alpha task")).toBeLessThan(miraText.indexOf("Zeta task"));
    expect(within(miraSection).getByLabelText("Agent running")).toBeInTheDocument();
  ...
  it("keeps default workspace topics in the Topics section instead of a project folder", () => {
    const sessions = [
      session({
        ...
        updatedAt: "2026-05-21T10:00:00Z",
        workspaceScope: {
          project_path: "/Users/me/.mira/workspace",
  ...
  it("can collapse a project group and keeps project rename separate from chat titles", async () => {
    ...
    expect(onToggleGroup).toHaveBeenCalledWith("project:/Users/me/mira");
    ...
    expect(onRequestRenameProject).toHaveBeenCalledWith("/Users/me/mira", "Photos");
  });
