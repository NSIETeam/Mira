...
const WS_OPEN = 1;
const WS_CLOSING = 2;
const HOST_SOCKET_URL_PREFIX = "mira-host://";
 ...
 * - **Production**: off unless ``localStorage.setItem('mira_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('mira_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("mira_debug_ws")?.trim().toLowerCase() ?? "";
...
type KernelEventHandler = (ev: KernelEventPayload) => void;
...
export class miraClient {
  ...
  constructor(private options: miraClientOptions) {
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    if (kernelEvent.type === "error" && this.pendingNewChat) {
      clearTimeout(this.pendingNewChat.timer);
      const errorPayload = parsed as { detail?: string; reason?: string };
      const detail = typeof errorPayload.detail === "string" ? errorPayload.detail : "server error";
      const reason = typeof errorPayload.reason === "string" && errorPayload.reason ? `:${errorPayload.reason}` : "";
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - miraClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }
