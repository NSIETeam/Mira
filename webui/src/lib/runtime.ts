import type { KernelGuiSurface, RuntimeCapabilities, RuntimeSurface } from "./types";
...
export interface miraHostApi {
  ...
  onRuntimeStatus?(
    listener: (status: KernelRuntimeInfo["engine_status"]) => void,
  ): () => void;
}
...
export function toKernelGuiSurface(surface: string | null | undefined): KernelGuiSurface {
  return surface === "native" ? "native" : "browser";
}

export function toRuntimeSurface(surface: string | null | undefined): RuntimeSurface {
  return toKernelGuiSurface(surface);
}
...
export function createRuntimeHost(
  ...
  return createKernelHost(surface, capabilities);
}

export function getKernelHost(
  surface?: string | null,
  capabilities?: Partial<RuntimeCapabilities> | null,
): KernelHost {
  const api = getHostApi();
  const runtimeSurface =
    surface == null ? (api ? "native" : "browser") : toKernelGuiSurface(surface);
  return createKernelHost(runtimeSurface, capabilities);
}
...
export function getRuntimeHost(
  ...
  return getKernelHost(surface, capabilities);
}

export function isNativeRuntime(surface?: string | null): boolean {
  return getHostApi() !== null || toKernelGuiSurface(surface) === "native";
}
