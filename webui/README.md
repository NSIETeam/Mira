...
This directory contains the React/TypeScript source for the mira WebUI. If
you installed `mira` from PyPI and only want to use the bundled browser UI,
...
mira/web/dist/      build output served by the gateway
```

## Develop the WebUI (Vite HMR)

### 1. Install mira from source
...
```bash
mira_API_URL=http://127.0.0.1:9000 bun run dev
```
