...
from mira.providers.openai_responses import (
    consume_sse_with_reasoning,
    convert_messages,
    convert_tools,
)
from mira.providers.xai_oauth import (
    XAI_CLIENT_VERSION,
    XAIToken,
    get_xai_oauth_token,
)
...
def _build_headers(token: str, model: str) -> dict[str, str]:
    ...
    return {
        ...
        "User-Agent": f"mira/{__version__} (python)",
...
def _friendly_error(status_code: int, response_body: str | None = None) -> str:
    if status_code == 401:
        message = "xAI rejected the login. Sign in again with `mira provider login xai-grok`."
    elif status_code == 403:
        message = "This xAI account or subscription cannot access the Grok subscription endpoint."
    elif status_code == 426:
        message = "xAI requires a newer Grok client version. Update mira and try again."
