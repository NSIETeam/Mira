...
class MCPToolWrapper(_MCPWrapperBase):
    ...
    def _store_image_block(
        ...
        from mira.utils.artifacts import ArtifactError, store_generated_image_artifact
...
class MCPResourceWrapper(_MCPWrapperBase):
    """Wraps an MCP resource URI as a read-only mira Tool."""
...
class MCPPromptWrapper(_MCPWrapperBase):
    """Wraps an MCP prompt as a read-only mira Tool."""
...
async def reload_servers(state: Any, registry: ToolRegistry) -> dict[str, Any]:
    ...
    async with _reload_lock(state):
        ...
        try:
            ...
            return {
                "ok": False,
                "message": "Could not reload MCP config. Restart mira to pick up changes.",
                "requires_restart": True,
                "error": str(exc),
            }
        ...
        if failed:
            ...
            message = "MCP connections refreshed without restarting mira."
        else:
            message = "MCP config reloaded without restarting mira."
...
async def request_mcp_reload(bus: Any, *, timeout: float = 15.0) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "ok": False,
            "message": "MCP hot reload timed out. Restart mira to pick up changes.",
            "requires_restart": True,
        }
...
async def handle_runtime_control(state: Any, msg: InboundMessage, registry: ToolRegistry) -> bool:
    ...
    try:
        ...
        result = {
            "ok": False,
            "message": "MCP hot reload failed. Restart mira to pick up changes.",
            "requires_restart": True,
            "error": str(exc),
        }
