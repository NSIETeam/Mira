    ...
    async def _request_model(
        ...
        *,
        ...
        if timeout_s is None:
            ...
            # Set mira_LLM_TIMEOUT_S=0 to disable.
            raw = os.environ.get("mira_LLM_TIMEOUT_S", "300").strip()
            try:
                timeout_s = float(raw)
        ...
        # (mira_STREAM_IDLE_TIMEOUT_S), but a stream that keeps producing
        # very slow deltas can still run forever. Use a more generous wall-clock
        # timeout for streaming while preserving mira_LLM_TIMEOUT_S=0 as an
