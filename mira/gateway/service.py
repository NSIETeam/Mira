...
from mira.gateway import GatewayStartOptions, build_gateway_command
...
class GatewayServiceInstaller:
    ...
    def _install_systemd(
        ...
        *,
        ...
        content = _systemd_unit_content(
            description=f"mira Gateway ({options.name})",
            command=command,
    ...
    def _install_launchd(
        ...
        *,
        ...
        stdout_path = self.home / ".mira" / "logs" / f"{log_stem}.launchd.log"
        stderr_path = self.home / ".mira" / "logs" / f"{log_stem}.launchd.err.log"
        payload = {
            "Label": label,
...
def _launchd_label(name: str) -> str:
    if name.startswith("ai.mira."):
        return name
    suffix = _safe_service_name(name).removeprefix("mira-").replace("-", ".")
    return f"ai.mira.{suffix}"
