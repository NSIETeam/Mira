"""Optional mira feature discovery and enablement."""
...
from mira.channels.registry import channel_default_enabled
...
def optional_dependency_groups_from_metadata() -> dict[str, list[str] | None]:
    ...
    try:
        extras = metadata("mira").get_all("Provides-Extra") or []
        raw_requirements = requires("mira") or []
    ...
    *,
    ...
    if name in _BUNDLED_FEATURE_ALIASES:
        payload = optional_features_payload(
            last_action={
                "ok": True,
                "message": f"Feature '{name}' is included with mira",
                "enabled": True,
            }
        )
...
def disable_optional_feature(
    ...
    *,
    ...
    from mira.channels.registry import discover_plugins
    from mira.config.loader import get_config_path, load_config
