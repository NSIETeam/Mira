...
from mira.apps.protocol import app_manifest, compact_dict
from mira.config.paths import get_runtime_subdir
from mira.security.workspace_policy import is_path_within
...
_CATALOG_SOURCES = (
    ...
    ("extensions", mira_EXTENSION_REGISTRY_URL, mira_EXTENSION_RAW_BASE, False),
)
...
class CliAppManager:
    ...
    def _manifest_source(self, app: dict[str, Any]) -> str:
        ...
        if source == "extensions":
            return "mira-extension"
        return f"cli-anything:{source}"

    def _trust_registry(self, app: dict[str, Any]) -> str:
        return "mira-extension" if str(app.get("_source") or "") == "extensions" else "cli-anything"
        ...
        *,
        ...
        remove = compact_dict({
            ...
            "verification": (
                ...
                else ["mira_state_absent", "managed_paths_absent"]
            ),
        })
        return app_manifest(
            app_id=name,
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = _catalog_description(app) or f"Use {display} from mira."
        return f"""---
...
Use this skill when the user asks mira to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_mira_skill_note(self, content: str, app: dict[str, Any]) -> str:
        marker = "<!-- mira-cli-app-note -->"
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_mira_skill_note(content, app)
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from mira. {entry_point} "
                "is still available because it is managed outside mira."
            )
