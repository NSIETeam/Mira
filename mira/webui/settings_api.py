...
from mira.agent.tools.web import SEARCH_PROVIDER_OPTIONS
from mira.audio.transcription import resolve_transcription_config
from mira.audio.transcription_registry import (
    resolve_transcription_provider,
    transcription_provider_names,
)
from mira.config.loader import get_config_path, load_config, resolve_config_env_vars, save_config
from mira.config.schema import ModelPresetConfig, ProviderConfig
from mira.kernel.app import build_kernel_manifest
from mira.kernel.profile import get_profile
from mira.kernel.shell import default_engineering_shell, get_shell
from mira.providers.image_generation import (
    get_image_gen_provider,
    image_gen_provider_names,
)
from mira.providers.registry import PROVIDERS, create_dynamic_spec, find_by_name
from mira.security.network import is_loopback_host
from mira.security.workspace_access import workspace_sandbox_status
from mira.webui.token_usage import token_usage_payload
...
_DOCS_LATEST_URL = "https://mira.wiki/docs/latest"
...
def _docs_payload() -> dict[str, Any]:
    ...
    base_url = f"https://mira.wiki/docs/{docs_version}"
    return {
...
def _oauth_provider_status(spec: Any) -> dict[str, Any]:
    ...
    if spec.name == "github_copilot":
        try:
            from mira.providers.github_copilot_provider import get_github_copilot_login_status
        except Exception:
            return {
                "configured": False,
                "account": None,
    ...
    if spec.name == "xai_grok":
        try:
            from mira.providers.xai_oauth import get_xai_oauth_login_status
        except Exception:
            return {
                "configured": False,
                "account": None,
...
def login_oauth_provider(query: QueryParams) -> dict[str, Any]:
    ...
    if spec.name == "github_copilot":
        try:
            from mira.providers.github_copilot_provider import (
    ...
    if spec.name == "xai_grok":
        from mira.providers.xai_oauth import start_xai_oauth_login
...
def complete_oauth_provider(
    ...
    from mira.providers.xai_oauth import complete_xai_oauth_login
...
def logout_oauth_provider(query: QueryParams) -> dict[str, Any]:
    ...
    if spec.name == "openai_codex":
        ...
        from mira.providers.xai_oauth import logout_xai_oauth
