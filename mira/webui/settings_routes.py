...
from mira.agent.tools.image_generation import request_image_generation_reload
from mira.agent.tools.mcp import request_mcp_reload
from mira.api.runtime import ApiRuntime, ApiStartOptions, api_runtime_paths
from mira.bus.queue import MessageBus
from mira.channels._setup import channel_setup_spec
from mira.channels.connect import ChannelConnectError
...
from mira.config.loader import get_config_path, load_config, save_config
...
from mira.pairing import approve_code, deny_code, list_pending
from mira.webui.cli_apps_api import cli_apps_action, cli_apps_payload
from mira.webui.http_utils import case_insensitive_header
from mira.webui.http_utils import is_local_browser_request as _is_local_browser_request
from mira.webui.http_utils import query_first as _query_first
from mira.webui.mcp_presets_api import mcp_presets_settings_action
from mira.webui.mira_features_api import (
    mira_feature_instance_target,
    mira_features_action,
    mira_features_payload,
)
from mira.webui.settings_api import (
    WebUISettingsError,
...
from mira.webui.version_check import check_for_update

QueryParams = dict[str, list[str]]

_MCP_VALUES_HEADER = "X-mira-MCP-Values"
_MCP_VALUES_HEADER_MAX_BYTES = 64 * 1024
_PROVIDER_VALUES_HEADER = "X-mira-Provider-Values"
_PROVIDER_VALUES_HEADER_MAX_BYTES = 64 * 1024
_CHANNEL_VALUES_HEADER = "X-mira-Channel-Values"
_CHANNEL_VALUES_HEADER_MAX_BYTES = 64 * 1024
_API_SERVICE_VALUES_HEADER = "X-mira-API-Service-Values"
_API_SERVICE_VALUES_HEADER_MAX_BYTES = 8 * 1024
_OAUTH_CODE_HEADER = "X-mira-OAuth-Code"
...
class WebUISettingsRouter:
    ...
    async def dispatch(self, connection: Any, request: WsRequest, path: str) -> Response | None:
        ...
        if path == "/api/settings/mira-features":
            return await self._handle_settings_mira_features(request)
        if path == "/api/settings/mira-features/enable":
            return await self._handle_settings_mira_features_action(connection, request, "enable")
        if path == "/api/settings/mira-features/disable":
            return await self._handle_settings_mira_features_action(connection, request, "disable")
    ...
    async def _handle_settings_mira_features(self, request: WsRequest) -> Response:
        if not self._authorized(request):
            return self._unauthorized()
        try:
            payload = await asyncio.to_thread(mira_features_payload)
        except Exception:
            self.logger.exception("failed to load mira features")
            return self._error_response(500, "failed to load mira features")
        return self._json_response(self._with_channel_runtime_status(payload))

    async def _handle_settings_mira_features_action(
        self,
        connection: Any,
        request: WsRequest,
        action: str,
        ...
        try:
            ...
            message = getattr(e, "message", str(e))
            if status >= 500:
                self.logger.exception("mira feature action '{}' failed", action)
            return self._error_response(status, message)
        payload = await self._apply_mira_feature_runtime_change(
            action,
            self._query(request),
            payload,
        )
    ...
    async def _apply_mira_feature_runtime_change(
        self,
        action: str,
        query: QueryParams,
        payload: dict[str, Any],
        ...
        try:
            instance_id = mira_feature_instance_target(query)
    ...
    async def _handle_settings_channel_configure(
        ...
        if not enable:
            features = await asyncio.to_thread(mira_features_payload)
            features = self._with_channel_runtime_status(features)
            payload["mira_features"] = self._with_restart_state(features, section="runtime")
            return self._json_response(payload)
        ...
        features = await self._apply_mira_feature_runtime_change(
            "enable",
            feature_query,
            features,
        )
