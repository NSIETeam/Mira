...
from mira.agent.goal_permission import goal_mutation_permission
...
from mira.utils.restart import set_restart_notice_to_env
from mira.utils.workspace_prompts import initialize_workspace_prompt
...
async def cmd_restart(ctx: CommandContext) -> OutboundMessage:
    ...
    async def _do_restart():
        ...
        argv = [sys.executable, "-m", "mira"] + sys.argv[1:]
...
async def cmd_status(ctx: CommandContext) -> OutboundMessage:
    ...
    with suppress(Exception):
        from mira.utils.searchusage import fetch_search_usage
    ...
    if args == "init":
        if not initialize_workspace_prompt(path, store.default_dream_prompt()):
            content = (
                ...
                "Edit that file, or delete/empty it to return to mira's default."
            )
        else:
            content = (
                f"Created Dream memory instructions at `{display_path}`.\n\n"
                "Edit that file to teach Dream how to organize memory. "
                "This fully replaces mira's default Dream guide for this workspace. "
                "Delete or empty it to return to mira's default."
            )
    elif args:
        ...
        content = (
            ...
            f"- Path: `{display_path}`\n"
            "- Delete or empty this file to return to mira's default."
        )
    else:
        content = (
            "Dream memory instructions: mira default\n\n"
...
async def cmd_evaluator_prompt(ctx: CommandContext) -> OutboundMessage:
    ...
    from mira.utils.evaluator import (
    ...
    if args == "init":
        ...
        content = (
            "Heartbeat evaluator prompt: mira default\n\n"
            f"- Editable file: `{display_path}`\n"
...
async def cmd_pairing(ctx: CommandContext) -> OutboundMessage:
    """List, approve, deny or revoke pairing requests."""
    from mira.pairing import PAIRING_COMMAND_META_KEY, handle_pairing_command
...
async def cmd_trigger(ctx: CommandContext) -> OutboundMessage:
    ...
    command = f'mira trigger {trigger.id} "message"'
    return OutboundMessage(
        channel=ctx.msg.channel,
        chat_id=ctx.msg.chat_id,
        content=(
