---
name: update-setup
description: "One-time setup wizard for the mira upgrade skill. Triggers: setup update, configure update, 设置更新, 初始化更新."
---

# Update Setup

Generate a personalized upgrade skill in mira's agent workspace.
...
Use `exec` to run `mira --version`. Tell the user the current version.
...
```
command -v mira || true
python -m pip show mira || true
pipx list | sed -n '/mira/,+3p' || true
uv tool list | sed -n '/mira/,+3p' || true
```
...
answer, stop and ask the user to rerun this setup when they know how mira was
installed.
...
`question: "Where is your mira source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "mira[EXTRAS]" --force` |
| pipx | `pipx install --force "mira[EXTRAS]"` |
| pip | `python -m pip install --upgrade "mira[EXTRAS]"` |
...
---
name: update
description: "Upgrade mira to the latest version. Triggers: upgrade mira, update mira, 升级mira, 更新mira."
---
...
4. Use `exec` to verify: `mira --version`
5. Tell the user the new version. Say: "Run `/restart` to restart mira and apply the update. If `/restart` is unavailable in this channel, restart the mira process manually."
```

## Step 5: Confirm

Only after `write_file` succeeds, tell the user:
"Upgrade skill created. Say 'upgrade mira' when you want to update."
