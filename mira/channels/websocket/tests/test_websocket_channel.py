...
from mira.bus.events import OUTBOUND_META_AGENT_UI, OutboundMessage
...
from mira.channels.websocket.runtime import (
    WebSocketChannel,
    WebSocketConfig,
    _is_valid_chat_id,
    _parse_envelope,
...
from mira.config.schema import Config, ModelPresetConfig
from mira.runtime_context import RUNTIME_CONTEXT_INPUT_META, WEBUI_QUOTE_SOURCE
...
from mira.webui.http_utils import (
...
from mira.webui.settings_api import settings_payload, update_provider_settings
from mira.webui.transcript import append_transcript_object, read_transcript_lines
...
async def test_start_extends_http_open_timeout_for_slow_settings_routes(
    ...
    import mira.channels.websocket.runtime as websocket_module
...
def isolate_webui_workspace_state(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "mira.webui.workspaces.get_webui_dir",
...
def test_issue_route_secret_matches_bearer_and_header() -> None:
    ...
    x_headers = Headers([("X-mira-Auth", "my-secret")])
...
async def test_webui_message_envelope_marks_inbound_metadata(bus: MagicMock) -> None:
    from mira.webui.transcript import read_transcript_lines
...
async def test_webui_message_envelope_persists_user_transcript_for_refresh(
    ...
    from mira.webui.transcript import build_webui_thread_response, read_transcript_lines
...
async def test_webui_user_transcript_append_failure_does_not_block_inbound(
    ...
    monkeypatch.setattr("mira.webui.transcript.append_transcript_object", fail_append)
    channel = _ch(bus)
    conn = AsyncMock()
    conn.remote_address = ("127.0.0.1", 50123)
...
def test_registry_discovers_websocket_channel() -> None:
    from mira.channels.registry import load_channel_class
...
async def test_settings_api_returns_safe_subset_and_updates_whitelist(
    ...
    monkeypatch.setattr(
        "mira.webui.settings_api._oauth_provider_status",
        lambda _spec: {
            "configured": False,
            "account": None,
            "expires_at": None,
    ...
    monkeypatch.setattr(
        "mira.webui.settings_routes.request_image_generation_reload",
        image_reload,
    )
    ...
    try:
        ...
        assert workspace_path.endswith("/.mira/workspace")
        ...
        custom_provider_created = await _http_get(
            ...
            headers={
                ...
                "X-mira-Provider-Values": json.dumps(
                    {
...
async def test_bootstrap_exposes_native_surface(bus: MagicMock) -> None:
    ...
    try:
        response = await _http_get(
            ...
            headers={"X-mira-Auth": "native-secret"},
        )
        assert response.status_code == 200
        body = response.json()
...
def test_settings_payload_reports_workspace_sandbox(monkeypatch, tmp_path) -> None:
    ...
    save_config(config, config_path)
    monkeypatch.setattr("mira.config.loader._current_config_path", config_path)
    monkeypatch.setenv("mira_SANDBOX_ENFORCED", "macos_app_sandbox")
...
def test_sessions_list_includes_active_run_started_at(monkeypatch) -> None:
    ...
    from mira.session import webui_turns as wth
    from mira.webui import ws_http as ws_http_module
...
def test_handle_file_preview_returns_workspace_file(tmp_path) -> None:
    ...
    workspace = tmp_path / "workspace"
    source = workspace / "mira" / "agent" / "hook.py"
    ...
    enc = quote(key, safe="")
    path = quote("mira/agent/hook.py:12", safe="")
    req = Request(
    ...
    assert resp.status_code == 200
    body = json.loads(resp.body.decode())
    assert body["display_path"] == "mira/agent/hook.py"
...
def test_file_preview_normalizes_windows_file_url() -> None:
    from mira.webui.file_preview import _clean_preview_path
