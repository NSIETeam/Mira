...
from mira.channels.websocket.runtime import WebSocketChannel, WebSocketConfig
from mira.cron.service import CronService
from mira.cron.types import CronJob, CronPayload, CronSchedule
...
from mira.webui.gateway_services import GatewayServices, build_gateway_services
...
async def test_session_automations_route_lists_local_triggers(
    ...
    try:
        ...
        assert job["payload"]["command"] == f'mira trigger {trigger.id} "message"'
        assert job["state"]["pending"] is True
    finally:
        await channel.stop()
        await server_task
...
async def test_webui_skills_route_requires_token_and_hides_paths(
    ...
    (unavailable_skill / "SKILL.md").write_text(
        "\n".join([
            ...
            "    requires:",
            "      bins:",
            "        - definitely-missing-mira-skill-cli",
            "      env:",
            "        - DEFINITELY_MISSING_mira_SKILL_ENV",
            "---",
    ...
    try:
        ...
        assert unavailable["unavailable_reason"] == (
            "CLI: definitely-missing-mira-skill-cli, "
            "ENV: DEFINITELY_MISSING_mira_SKILL_ENV"
        )
        ...
        assert detail_body["requirements"] == {
            "bins": ["definitely-missing-mira-skill-cli"],
            "env": ["DEFINITELY_MISSING_mira_SKILL_ENV"],
            "missing_bins": ["definitely-missing-mira-skill-cli"],
            "missing_env": ["DEFINITELY_MISSING_mira_SKILL_ENV"],
        }
...
async def test_cli_apps_routes_require_token_and_return_payload(
    ...
    monkeypatch.setattr(
        "mira.webui.settings_routes.cli_apps_payload",
        payload,
    )
    monkeypatch.setattr(
        "mira.webui.settings_routes.cli_apps_action",
        lambda action, query: {
            "apps": [],
            "installed_count": 1,
            "catalog_updated_at": "2026-04-18",
...
@pytest.mark.asyncio
async def test_mira_feature_routes_require_token_and_enable(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    server_task = asyncio.create_task(channel.start())
    try:
        deny = await _http_get("http://127.0.0.1:29916/api/settings/mira-features")
        assert deny.status_code == 401
        ...
        catalog = await _http_get(
            "http://127.0.0.1:29916/api/settings/mira-features",
            headers=auth,
        )
        assert catalog.status_code == 200
        ...
        enabled = await _http_get(
            "http://127.0.0.1:29916/api/settings/mira-features/enable?name=matrix",
            headers=auth,
        )
        ...
        disabled_websocket = await _http_get(
            "http://127.0.0.1:29916/api/settings/mira-features/disable?name=websocket",
            headers=auth,
        )
        ...
        disabled = await _http_get(
            "http://127.0.0.1:29916/api/settings/mira-features/disable?name=matrix",
            headers=auth,
        )
...
@pytest.mark.asyncio
async def test_mira_feature_route_reports_live_channel_failure(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    try:
        ...
        response = await _http_get(
            "http://127.0.0.1:29946/api/settings/mira-features",
            headers={"Authorization": f"Bearer {token}"},
        )
...
async def test_pairing_routes_require_token_and_approve_or_deny(
    ...
    monkeypatch.setattr("mira.webui.settings_routes.list_pending", lambda: list(pending))
    monkeypatch.setattr(
        "mira.webui.settings_routes.approve_code",
        lambda code: approved.append(code) or ("feishu", "ou_123") if code == "ABCD-EFGH" else None,
    )
    monkeypatch.setattr(
        "mira.webui.settings_routes.deny_code",
...
def test_api_service_settings_read_api_key_from_private_header(bus: MagicMock) -> None:
    channel = _ch(bus)
    request = _FakeReq(
        {"X-mira-API-Service-Values": json.dumps({"api_key": "secret-token"})},
...
def test_api_service_settings_reject_invalid_private_header(bus: MagicMock) -> None:
    ...
    channel = _ch(bus)
    request = _FakeReq(
        {"X-mira-API-Service-Values": json.dumps({"api_key": 123})},
...
@pytest.mark.asyncio
async def test_mira_feature_remote_install_requires_opt_in(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_mira_feature_local_install_allowed_by_default(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_mira_feature_channel_action_can_apply_without_restart(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
async def test_channel_connect_runtime_import_error_is_not_reported_as_unsupported(
    ...
    monkeypatch.setattr(
        "mira.webui.settings_routes.load_channel_plugin",
...
async def test_feishu_connect_routes_write_config_and_hot_reload(
    ...
    monkeypatch.setattr(
        "mira.webui.settings_routes.mira_features_action",
    ...
    assert body["mira_features"]["requires_restart"] is False
...
def test_feishu_connect_create_appends_instance(
    ...
    from mira.channels.feishu import runtime as feishu_module
    from mira.channels.feishu.connect import FeishuConnectStore
    from mira.config import loader
...
async def test_channel_configure_route_saves_discord_config_and_hot_reloads(
    ...
    monkeypatch.setattr("mira.webui.settings_routes.mira_features_action", fake_feature_action)
    calls: list[tuple[str, str, str]] = []
        ...
        _FakeReq(
            {
                ...
                "Host": "127.0.0.1:8765",
                "X-mira-Channel-Values": json.dumps(
                    {
...
async def test_channel_configure_route_preserves_existing_channel_values(
    ...
    discord = next(
        feature
        for feature in body["mira_features"]["features"]
...
@pytest.mark.asyncio
async def test_mira_feature_loopback_reverse_proxy_install_requires_opt_in(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_mira_feature_remote_enable_without_install_is_allowed(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_mira_feature_remote_disable_does_not_need_install_policy(
    bus: MagicMock,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    request = _FakeReq(
        {"Authorization": f"Bearer {token}"},
        path="/api/settings/mira-features/disable?name=matrix",
    )

    response = await channel.gateway.http.settings_routes.dispatch(
        _REMOTE,
        request,
        "/api/settings/mira-features/disable",
    )
...
async def test_cli_apps_catalog_does_not_block_other_webui_http_routes(
    ...
    monkeypatch.setattr("mira.webui.settings_routes.cli_apps_payload", slow_payload)
...
async def test_cli_apps_route_supports_installed_only_payload(
    ...
    monkeypatch.setattr("mira.webui.settings_routes.cli_apps_payload", payload)
...
async def test_mcp_presets_routes_require_token_and_return_payload(
    ...
    monkeypatch.setattr(
        "mira.webui.mcp_presets_api.mcp_presets_payload",
        lambda: {
            "presets": [
                {
                    "name": "browserbase",
    ...
    monkeypatch.setattr(
        "mira.webui.mcp_presets_api.mcp_presets_action",
        _mcp_preset_action,
    )
    monkeypatch.setattr(
        "mira.webui.mcp_presets_api.custom_mcp_action",
        _custom_action,
    )
    ...
    monkeypatch.setattr(
        "mira.webui.settings_routes.request_mcp_reload",
        _hot_reload,
    )
    ...
    try:
        ...
        bad_header = await _http_get(
            "http://127.0.0.1:29913/api/settings/mcp-presets/enable?name=browserbase",
            headers={**auth, "X-mira-MCP-Values": "[]"},
        )
        ...
        imported = await _http_get(
            ...
            headers={**auth, "X-mira-MCP-Values": json.dumps({"config": "{}"})},
        )
...
async def test_session_delete_removes_file(
    ...
    from mira.webui.transcript import append_transcript_object
...
async def test_webui_automations_route_lists_all_jobs_and_allows_user_actions(
    ...
    try:
        ...
        updated = await _http_get(
            ...
            headers={
                **auth,
                "X-mira-Automation-Values": json.dumps(
                    {
                        "name": "Daily quiz",
        ...
        unicode_update = await _http_get(
            ...
            headers={
                **auth,
                "X-mira-Automation-Values": quote(
                    json.dumps(
                        {
                            "name": "每日测验",
                            "message": "问今日测验",
        ...
        malformed_update = await _http_get(
            ...
            headers={
                **auth,
                "X-mira-Automation-Values": json.dumps({"message": ["bad"]}),
            },
        )
        ...
        protected_update = await _http_get(
            ...
            headers={
                **auth,
                "X-mira-Automation-Values": json.dumps({"name": "bad"}),
            },
        )
...
async def test_webui_automations_route_manages_local_triggers(
    ...
    try:
        ...
        assert by_id[trigger.id]["trigger"]["command"] == f'mira trigger {trigger.id} "message"'
        ...
        renamed = await _http_get(
            ...
            headers={
                **auth,
                "X-mira-Automation-Values": json.dumps({"name": "Release review"}),
            },
        )
        ...
        bad_update = await _http_get(
            ...
            headers={
                **auth,
                "X-mira-Automation-Values": json.dumps({"message": "coupled"}),
            },
        )
...
async def test_webui_thread_resigns_assistant_media_urls(
    ...
    monkeypatch.setattr("mira.webui.media_gateway.get_media_dir", fake_media_dir)
...
def test_local_browser_request_requires_loopback_host_and_forwarded_origin() -> None:
    from mira.webui.http_utils import is_local_browser_request
    ...
    assert is_local_browser_request(_LOCAL, {"Host": "mira.example"}) is False
    assert (
        is_local_browser_request(
            _LOCAL,
    ...
    assert (
        is_local_browser_request(
            _LOCAL,
            {"Host": "127.0.0.1:8765", "X-Forwarded-Host": "mira.example"},
        )
        is False
    )
    assert (
        is_local_browser_request(
            _LOCAL,
            {"Host": "127.0.0.1:8765", "Forwarded": "for=203.0.113.42;host=mira.example"},
        )
        is False
    )
...
def test_wildcard_ipv6_with_secret_is_valid(bus: MagicMock) -> None:
    ...
    resp = channel.gateway.http._handle_bootstrap(
        _REMOTE, _FakeReq({"X-mira-Auth": "s3cret"})
    )
    assert resp.status_code == 200
...
def test_bootstrap_ws_url_uses_forwarded_https_host(bus: MagicMock) -> None:
    ...
    assert resp.status_code == 200
    body = json.loads(resp.body)
    assert body["ws_url"] == "wss://mira.example/"
...
def test_bootstrap_without_auth_rejects_reverse_proxy_remote_headers(bus: MagicMock) -> None:
    ...
    resp = channel.gateway.http._handle_bootstrap(
        _LOCAL,
        _FakeReq({"Host": "mira.example", "X-Forwarded-For": "203.0.113.42"}),
    )
    assert resp.status_code == 403
...
def test_bootstrap_prefers_runtime_model_name(bus: MagicMock, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "mira.webui.ws_http._default_model_name_from_config",
        lambda: "from-disk",
    )
...
def test_bootstrap_accepts_x_mira_auth_header(bus: MagicMock) -> None:
    channel = _ch(bus, host="0.0.0.0", tokenIssueSecret="s3cret")
    resp = channel.gateway.http._handle_bootstrap(
        _REMOTE, _FakeReq({"X-mira-Auth": "s3cret"})
    )
