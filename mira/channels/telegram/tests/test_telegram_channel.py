...
from mira.channels.telegram.runtime import (
...
async def test_start_creates_separate_pools_with_proxy(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mira.channels.telegram.runtime.HTTPXRequest", _FakeHTTPXRequest)
    monkeypatch.setattr(
        "mira.channels.telegram.runtime.Application",
...
async def test_send_text_retries_on_timeout() -> None:
    ...
    import mira.channels.telegram.runtime as tg_mod
...
async def test_send_delta_stream_end_does_not_fallback_on_network_timeout(
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr("mira.channels.telegram.runtime._SEND_RETRY_BASE_DELAY", 0)
...
async def test_send_delta_stream_end_html_expansion_does_not_overflow() -> None:
    ...
    """
    from mira.channels.telegram.runtime import _markdown_to_telegram_html
...
async def test_send_delta_incremental_html_expansion_does_not_overflow() -> None:
    ...
    from mira.channels.telegram.runtime import TELEGRAM_HTML_MAX_LEN
...
async def test_send_remote_media_url_after_security_validation(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr("mira.channels.telegram.runtime.validate_url_target", lambda url: (True, ""))
...
async def test_send_blocks_unsafe_remote_media_url(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr(
        "mira.channels.telegram.runtime.validate_url_target",
...
async def test_download_message_media_returns_path_when_download_succeeds(
    ...
    monkeypatch.setattr(
        "mira.channels.telegram.runtime.get_media_dir",
...
async def test_forward_command_pairs_unauthorized_private_user(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr(
        "mira.channels.base.generate_code", lambda _ch, _sid: "ABCD-EFGH"
    )
...
async def test_forward_command_normalizes_telegram_safe_dream_aliases() -> None:
    ...
    handled.clear()
    update = _make_telegram_update(text="/dream_prompt@mira_test init", reply_to_message=None)
...
def test_telegram_bus_slash_command_regex_matches_agent_loop_commands() -> None:
    ...
    assert pat.fullmatch("/skill@mira_bot")
    assert pat.fullmatch("/new@mira_bot")
    assert pat.fullmatch("/goal@mira_bot refine objective")
    assert pat.fullmatch("/trigger@mira_bot CI summary")
...
def test_strip_md_block_removes_inline_formatting() -> None:
    from mira.channels.telegram.runtime import _strip_md_block
