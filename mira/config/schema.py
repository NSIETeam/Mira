...
from mira.config_base import Base
from mira.cron.types import CronSchedule

if TYPE_CHECKING:
    from mira.agent.tools.cli_apps import CliAppsToolConfig
    from mira.agent.tools.filesystem import FileToolsConfig
    from mira.agent.tools.image_generation import ImageGenerationToolConfig
    from mira.agent.tools.self import MyToolConfig
    from mira.agent.tools.shell import ExecToolConfig
    from mira.agent.tools.web import WebToolsConfig
...
class TranscriptionConfig(Base):
    ...
    enabled: bool = True
    provider: str | None = None  # Validated by mira.audio.transcription_registry.
    model: str | None = None
...
class ProviderConfig(Base):
    ...
    # mira/providers/openai_compat_provider.py. Kept duplicated here to
...
class ToolsConfig(Base):
    ...
    web: WebToolsConfig = Field(default_factory=lambda: _lazy_default("mira.agent.tools.web", "WebToolsConfig"))
    exec: ExecToolConfig = Field(default_factory=lambda: _lazy_default("mira.agent.tools.shell", "ExecToolConfig"))
    file: FileToolsConfig = Field(default_factory=lambda: _lazy_default("mira.agent.tools.filesystem", "FileToolsConfig"))
    cli_apps: CliAppsToolConfig = Field(default_factory=lambda: _lazy_default("mira.agent.tools.cli_apps", "CliAppsToolConfig"))
    my: MyToolConfig = Field(default_factory=lambda: _lazy_default("mira.agent.tools.self", "MyToolConfig"))
    image_generation: ImageGenerationToolConfig = Field(
        default_factory=lambda: _lazy_default("mira.agent.tools.image_generation", "ImageGenerationToolConfig"),
    )
...
def _resolve_tool_config_refs() -> None:
    ...
    like ``from mira.config.schema import ExecToolConfig`` continue to work.
    """
    import sys
    ...
    from mira.agent.tools.self import MyToolConfig
    from mira.agent.tools.shell import ExecToolConfig
    from mira.agent.tools.web import WebFetchConfig, WebSearchConfig, WebToolsConfig
