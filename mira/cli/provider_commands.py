"""Provider OAuth CLI command registration."""
...
def _resolve_oauth_provider(provider: str):
    ...
    if not spec:
        ...
        _get_console().print(f"[red]Unknown OAuth provider: {provider}[/red]  Supported: {names}")
        raise typer.Exit(1)
    return spec
...
def _set_oauth_provider_as_main(
    ...
    *,
    ...
    selected_model = (model or "").strip() or OAUTH_PROVIDER_DEFAULT_MODELS[provider_name]
...
def register_provider_commands(app: typer.Typer, *, console: Console) -> None:
    """Register OAuth provider management commands."""
    global _console
    _console = console
    ...
    def provider_logout(
        ...
        handler = LOGOUT_HANDLERS.get(spec.name)
        if not handler:
...
def _logout_openai_codex() -> None:
    ...
    _delete_oauth_files(storage.get_token_path(), PROVIDER_DISPLAY["openai_codex"])
...
def _login_xai_grok() -> None:
    ...
    from mira.config.loader import load_config, resolve_config_env_vars
    from mira.providers.xai_oauth import get_xai_oauth_token, login_xai_oauth
...
def _logout_xai_grok() -> None:
    ...
    from mira.providers.xai_oauth import get_xai_oauth_storage_path, logout_xai_oauth

    console = _get_console()
    token_path = get_xai_oauth_storage_path()
    provider_label = PROVIDER_DISPLAY["xai_grok"]
    if logout_xai_oauth():
...
def _logout_github_copilot() -> None:
    ...
    storage = get_storage()
    _delete_oauth_files(storage.get_token_path(), PROVIDER_DISPLAY["github_copilot"])
...
def _login_github_copilot() -> None:
    console = _get_console()
    try:
        from mira.providers.github_copilot_provider import login_github_copilot
