# AI Agent Memory in mira

This page explains how mira implements long-term AI agent memory: session
...
is `~/.mira/workspace/`, or the path passed with `--workspace`). Selecting a
...
Dream decides what to keep, update, or forget using mira's built-in memory instructions. Most users can leave this alone.
...
To return to mira's default behavior, delete `prompts/dream.md` or leave it empty.

Each workspace has its own guide. Changing this file does not affect other mira workspaces.
