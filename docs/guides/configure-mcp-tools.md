# How to Configure MCP Tools in mira

This guide adds an MCP server to mira so the agent can use external tools
through the Model Context Protocol.

## What you will build

- a working mira agent
- one MCP integration configured through Apps or `~/.mira/config.json`
...
you want external tools to be managed outside mira core.
...
1. Run `mira webui` and open **Apps**.
...
For manual or deployment-managed config, add this to `~/.mira/config.json`:
...
Restart mira and ask a question that requires the MCP tool.
...
- HTTP/SSE MCP uses mira's SSRF guard.
...
- Run the MCP command outside mira first.
- Start `mira gateway --verbose` and inspect tool registration logs.
