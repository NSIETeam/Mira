# Build a WhatsApp AI Agent with mira

This guide connects mira to WhatsApp through the `whatsapp` channel. The
channel links as a WhatsApp device and uses the same mira agent runtime,
...
- A machine that can keep `mira gateway` running.
...
- If QR linking fails, rerun `mira channels login whatsapp`.
- If you are migrating from the old bridge, remove `bridgeUrl` and
  `bridgeToken`, then re-login.
- If a sender appears as a LID instead of a phone number, let mira learn the
