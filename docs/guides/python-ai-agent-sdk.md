...
This guide shows when to use the mira Python SDK instead of calling a model
...
- a Python script that creates a `mira`
...
- Reuse one `mira` instance for related work.
...
- If SDK code fails, first run `mira agent -m "Hello!"` in the same
  environment.
