# How to Run a Self-Hosted AI Agent with mira

This guide sets up mira as a self-hosted AI agent runtime on your own
machine or server. The result is a gateway process that can serve the WebUI,
chat apps, automations, and API integrations.

## What you will build

- a mira config and workspace under your control
- a model provider connected through `config.json`
- a long-running `mira gateway`
...
Or connect a channel in `~/.mira/config.json`, then keep the same gateway
...
- Run `mira status` with the same `--config` and `--workspace` flags used by
  the service.
- Run `mira gateway --verbose` while debugging channel startup.
