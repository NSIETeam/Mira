# How to Add MCP Tools to an AI Agent with mira

mira can connect MCP servers and expose their tools to the agent alongside
...
publishes an MCP adapter, or when you want a clean boundary between mira and
external tool logic.
...
Add a stdio MCP server to `~/.mira/config.json`:
...
Restart mira, then ask a question that needs the MCP tool.
...
- Keep MCP server install/update steps outside mira config when possible.
...
- Start `mira gateway --verbose` and check MCP startup logs.
- Confirm the MCP command works by itself before debugging mira.
