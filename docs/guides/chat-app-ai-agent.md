# How to Connect an AI Agent to Chat Apps with mira

mira can run as a self-hosted chatbot or AI agent in Telegram, Discord,
...
The `mira webui` command already runs the gateway. For a chat-only or server deployment, start it directly:
...
- If `mira channels status` does not show the channel, the config key or
...
- If messages do not arrive, run `mira gateway --verbose` and compare
