# How to Build a Personal AI Agent with mira
...
chat-only interface. mira is useful when the agent needs local workspace
...
The wizard creates `~/.mira/config.json` and helps you choose a provider and
...
- Use `modelPresets` when you want stable names for fast, deep, local, or
  fallback models.
- Keep `mira gateway` running for WebUI, chat apps, automations, and the
  WebSocket channel.
...
- `mira status` shows the config path, workspace path, and active model.
- If `mira agent -m "Hello!"` fails, fix provider setup before opening the
  WebUI or chat apps.
