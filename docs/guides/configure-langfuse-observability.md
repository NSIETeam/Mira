# How to Configure Langfuse Observability for mira

mira can trace supported OpenAI-compatible provider calls through Langfuse's
OpenAI SDK wrapper.

## What you will build

- Langfuse installed in the same Python environment as mira
- Langfuse environment variables set before startup
- one traced mira model call
...
Set credentials before starting mira:
...
- Run one local `mira agent -m "Hello!"` call before debugging service logs.
