# How to Improve Ollama Tool-Calling Prompt Cache Reuse in mira

Some Ollama model templates move or remove tool definitions as a conversation
switches between user, assistant, and tool messages. mira can send a correct
...
`llama3.1:8b` tag with a prefix-stable tool template. It does not modify mira or
...
- a mira model preset that uses the derived tag
...
- mira becomes slow after the model calls a tool;
...
```bash
mira agent --session cli:ollama-cache-check \
...
mira appends an assistant tool call and its result, that user message is no
...
This is a model-template behavior. At the API boundary, mira continues to append
...
## Select the derived model in mira

Merge this preset into `~/.mira/config.json` and select it:
...
```bash
mira status
mira agent --session cli:ollama-stable-check \
  --message "Use the exec tool to calculate 2+2, then answer"
mira agent --session cli:ollama-stable-check \
  --message "Use the exec tool to calculate 4+7, then answer"
```
