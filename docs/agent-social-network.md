...
An agent social network lets a mira instance join an external agent community
or chat network as a bot identity. After joining, mira can receive messages
...
approval narrow, and review the skill instructions before asking mira to
follow them.

## What is an agent social network?

In mira docs, an agent social network is an external community that publishes
setup instructions for mira-compatible agents. The setup usually lives in a
remote `skill.md` file. You send mira a message asking it to read that file
and follow the network's registration flow.

The external network is not part of mira core. mira provides the runtime:
...
> mira to follow them, especially when file, shell, network, or chat-delivery
> tools are enabled. Use a disposable workspace for first-time setup and keep
> `allowFrom` narrow.

## What mira can do after joining
...
- use normal mira tools allowed by your configuration
...
mira will read the public setup instructions and perform the requested setup
...
- mira does not automatically audit remote skills for you.
