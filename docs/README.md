...
| You are comfortable running commands | [Install and Quick Start](./quick-start.md) | `mira status` is healthy and the WebUI or CLI can get one reply |
...
1. Install Mira.
2. Let the installer open `mira webui` on a fresh local desktop.
3. Configure a provider and model in **Settings → Models**.
4. Send `Hello!` before configuring anything else.

Most people do not need to edit JSON for the first run. The WebUI handles the initial provider, model, and local browser settings. SSH, headless, existing-config, and older-release installs retain `mira onboard --wizard` as a terminal fallback. After the WebUI opens, use **Settings** for models and built-in capabilities, **Settings → Channels** for chat apps, and **Apps** for CLI App or MCP integrations.
...
| Understand the Mira engineering workbench | [Mira Kernel Workbench](./mira-kernel-workbench.md) |
...
| Run Mira continuously | [Deployment](./deployment.md) |
| Prepare the current Mira workbench for release | [Mira Delivery Readiness](./mira-delivery-readiness.md) |
| Run separate bots or workspaces | [Multiple Instances](./multiple-instances.md) |
| Call Mira from Python | [Python SDK](./python-sdk.md) |
...
| Check release-prep status for the current Mira workbench | [Mira Delivery Readiness](./mira-delivery-readiness.md) |
...
These pages explain implementation and extension points. You do not need them to install or operate Mira.
...
If a command or screen no longer matches these docs, please [open an issue](https://github.com/NSIETeam/Mira/issues) with your Mira version, operating system, and the page that needs correction.
