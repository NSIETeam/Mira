...
Use this page when you want to understand mira before changing advanced settings. It explains the moving parts without requiring you to read the source first.

If you want source-file ownership and extension points, read [`architecture.md`](./architecture.md) after this page.

## Runtime Shape

mira has one small core loop and several ways to enter it:
...
The simplest path is `mira agent -m "Hello!"`: one inbound message goes through the agent loop and prints the reply in your terminal. The long-running path is `mira gateway`: channels receive messages from chat apps or the WebUI, publish them to the same agent loop, and send replies back to the originating channel.

## Config vs Workspace

The default instance lives under `~/.mira/`:

| Path | Meaning |
|---|---|
| `~/.mira/config.json` | Instance configuration: providers, model defaults, channels, tools, gateway, API, and runtime options |
| `~/.mira/workspace/` | Agent workspace: memory, sessions, heartbeat tasks, cron jobs, skills, and generated artifacts |

You can override both with command flags:

```bash
mira onboard --config ./bot-a/config.json --workspace ./bot-a/workspace
...
The config file controls what mira may use. The workspace is where mira keeps state for that instance.
...
`config.json` accepts both camelCase and snake_case keys. The docs use camelCase because mira writes config back to disk with camelCase aliases, for example `apiKey`, `modelPresets`, `intervalS`, and `maxToolResultChars`.

Most examples are partial snippets. Merge them into the existing file created by `mira onboard`; do not replace the whole file unless you want to reset the instance.
...
|---|---|---|
| CLI one-shot | `mira agent -m "..."` | First-run checks, scripts, and quick local questions |
| CLI interactive | `mira agent` | Terminal chat with persistent session history |
| Gateway | `mira gateway` | Chat apps, WebUI, heartbeat, Dream, and long-running service mode |
| OpenAI-compatible API | `mira serve` | Programmatic access through `/v1/chat/completions` |
| WebUI | `mira webui` | Prepare the local WebUI, start the gateway, and open the browser workbench |
...
1. If the active preset provider or implicit default provider is not `"auto"`, mira uses that provider.
2. If provider is `"auto"`, mira tries to infer the provider from the model name, configured API keys, local provider base URLs, or gateway providers.
...
When `mira gateway` starts, it runs workspace-scoped automations and
registers system jobs:

- `dream`, when `agents.defaults.dream.enabled` is true;
- `heartbeat`, when `gateway.heartbeat.enabled` is true.

Heartbeat reads `<workspace>/HEARTBEAT.md`. If the file has tasks under `## Active Tasks`, mira executes them and sends only useful/actionable results to the most recently active chat target. Routine "nothing changed" results are suppressed.
...
`mira trigger <id> "<message>"` when a local script or external service wants
mira to respond in that session. Webhook servers, third-party auth, and
event-to-message formatting stay outside mira. Trigger deliveries are stored
