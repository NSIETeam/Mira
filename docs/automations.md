...
<!-- Meta description: Create, run, and manage mira scheduled automations, local triggers, and heartbeat-backed background checks. -->

Automations are agent turns that run later in a linked topic. Use them
when mira should do work without someone actively typing: reminders,
...
result should appear. That lets mira keep the right session history,
...
|---|---|---|---|
| Scheduled automation | Time, interval, or cron expression | Recurring reminders, scheduled summaries, one-time future tasks | Ask mira in the target topic to schedule it with the `cron` tool |
| Local trigger | A local `mira trigger ...` command | CI jobs, webhooks, shell scripts, generated reports | `/trigger <name>` in the target topic |
...
Keep `mira gateway` running. The gateway owns background delivery for chat
...
local trigger messages. If you run multiple mira instances, pass the matching
`--config` or `--workspace` option to `mira trigger`.

Create each automation from the target topic. An automation without a linked
topic cannot be enabled or run from the WebUI because mira would not know
where to deliver the turn.

## Scheduled Automations

Scheduled automations are created by the agent's `cron` tool. In practice, ask
mira from the target chat or WebUI topic:
...
`America/Vancouver`; otherwise mira uses the runtime default timezone.
...
mira replies with a trigger ID and a command shaped like:

```bash
mira trigger trg_8K4P2Q9X "Review PR #4502"
```

Replace the quoted text with the message mira should receive. For generated
...
```bash
mira trigger --config ./bot-a/config.json trg_8K4P2Q9X "Nightly report"
mira trigger --workspace ./bot-a/workspace trg_8K4P2Q9X "Nightly report"
```

mira does not provide a built-in public webhook receiver for local triggers.
If GitHub, CI, or another external system should wake mira, run your own
small webhook service and have it call `mira trigger` after it builds the
final message.
...
Heartbeat is enabled by default when `mira gateway` starts. Configure it in
...
message. Copy the `mira trigger ...` command from the WebUI and replace
...
```bash
mira trigger <trigger-id> "Build failed on main. Inspect the logs and suggest the next fix."
```

For a local report script:

```bash
generate-report | mira trigger <trigger-id>
```

## Troubleshooting

If an automation does not run, check that `mira gateway` is running, the
...
- [`cli-reference.md#local-triggers`](./cli-reference.md#local-triggers) for `mira trigger`
