...
The published `mira` wheel already includes the WebUI bundle. You only need
...
`mira webui` creates the config/workspace when needed, enables the local
...
Complete first-time model setup in a foreground `mira webui` session before using
`--background`.

Manage the background gateway with `mira gateway status`, `mira gateway
logs`, `mira gateway restart`, and `mira gateway stop`.
...
Open Apps from the sidebar to manage tools that mira can attach to a chat
turn. The default **Ready** view shows only tools that can be used immediately:

- **Apps** are local command-line adapters that mira runs on your machine.
...
Apps intentionally does not list mira runtime support packages such as
...
included in mira and activate automatically when a file is attached. The
equivalent CLI for optional integrations remains `mira plugins`. See
...
extraction tools without requiring an API key. This does not replace mira's
...
It is an optional integration and does not replace mira's built-in web search
...
know whether mira already has a focused workflow for a task before you ask it
to perform that task.

## Automations

Automations are agent turns that run later in a linked topic. Create them from
the topic or channel where they are supposed to run so mira keeps the
...
- Local triggers, created with `/trigger <name>`, run when you call a local
  command such as `mira trigger trg_8K4P2Q9X "Review PR #4502"`.
...
because mira would not know where to deliver the scheduled turn. Recreate it
...
message. Use the copied `mira trigger ...` command and replace `"message"`
...
install missing mira support packages, such as adding a channel dependency,
...
trusted to change the Python environment that mira runs in. If you publish
...
1. `mira agent -m "Hello!"` works in the same Python environment.
2. `~/.mira/config.json` does not explicitly set `channels.websocket.enabled` to `false`.
3. `mira gateway` is still running.
