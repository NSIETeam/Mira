...
This walkthrough is for people who have not used a terminal, API key, or JSON config file before. The goal is only to get one reply in a browser. You do not need to understand mira's architecture or edit its config by hand.
...
| WebUI | The local browser page where you use mira |
...
mira does not create an AI provider account for you. Before setup, have these details nearby:
...
The installer downloads the stable mira package into an isolated Python environment. On a fresh local desktop, it then starts the WebUI and opens your browser. This can take a few minutes on the first run. Keep the terminal open. It prints the exact command used to run mira; if `mira` is not found later, reuse that whole command instead of switching to a different Python command.
...
If the installer finishes without opening the browser and `mira` is available, run:

```bash
mira webui
```

If the terminal cannot find `mira`, take the exact command printed by the installer and replace its final arguments with `webui`. That command may begin with `uv tool run`, `pipx run`, or the full path to mira's private Python environment.

On SSH, a computer without a desktop, an existing configuration, or an older mira release, the installer may open the terminal wizard instead. Choose **Quick Start** there and follow its prompts.
...
| Schedule a reminder or recurring task | Ask mira in the target chat, then manage it in **Automations** |
...
Some runtime changes ask you to restart mira. Use the restart action shown by the WebUI, or return to the terminal, press `Ctrl+C`, and run `mira webui` again.
...
|---|---|
| `mira: command not found` | Reuse the exact mira command printed by the installer; it points to the isolated environment that contains the package |
...
| Browser opens but messages fail | Test `mira agent -m "Hello!"` to separate a model problem from a WebUI problem |
| A change was saved but nothing changed | Restart mira so the running process reloads the config |

If you ask for help, include your operating system, `mira --version`, `mira status`, the exact command, and the exact error. Remove every API key, bot token, password, OAuth token, and private account ID first.
...
Leave that terminal open while you use mira. To stop it, return to the terminal and press `Ctrl+C`. Use `mira webui --background` only after the normal foreground start and model setup work; then manage it with `mira gateway status`, `logs`, `restart`, and `stop`.
