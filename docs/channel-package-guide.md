...
Use this guide to add a self-contained channel package to the mira repository. A channel is part of mira when its package lives at `mira/channels/<channel>/`; there is no separate external channel-plugin path.

> **Breaking change:** mira no longer discovers the `mira.channels` Python entry-point group. Move an entry-point implementation into `mira/channels/<channel>/` with a package-owned manifest, runtime, tests, and optional WebUI contribution.

## How It Works

When `mira gateway` starts, mira scans the packages under `mira/channels/` and loads each dependency-free `ChannelPlugin` descriptor from `manifest.py`.
...
```python
# mira/channels/webhook/__init__.py
"""Webhook channel package."""
```

```python
# mira/channels/webhook/manifest.py
from mira.channels.contracts import ChannelFieldSpec, ChannelSetupSpec
...
```python
# mira/channels/webhook/runtime.py
import asyncio
...
from mira.config.schema import Base
...
```bash
mira plugins list      # verify the channel package appears as "webhook"
mira onboard           # add default config for detected channels
```
...
Every channel is a self-contained package at `mira/channels/<channel>/`; channel-specific runtime code, setup metadata, tests, WebUI structure, components, and translations stay under that directory.

### Package Layout

```text
mira/channels/<channel>/
...
Do not add a runtime module directly under `mira/channels/`, create a parallel manifest tree, or add a central per-channel UI catalog. If existing channel files move, use `git mv` so history remains traceable.

### Manifest and Runtime Boundary

`manifest.py` exports a typed `ChannelPlugin` whose `runtime` target is an absolute import target, such as `mira.channels.telegram.runtime:TelegramChannel`; using `f"{__package__}.runtime:TelegramChannel"` keeps it package-owned without repeating the package path. Discovery imports the manifest before it knows whether the optional platform dependency is installed, so `manifest.py` must not import `runtime.py` or any platform SDK. Import runtime symbols from `runtime.py` explicitly; `__init__.py` remains an inert package marker.
...
Use the small constructors in [`mira/channels/_manifest.py`](../mira/channels/_manifest.py) for declarative field and requirement definitions. Use [`mira/channels/dingtalk/manifest.py`](../mira/channels/dingtalk/manifest.py) as a compact single-instance example and [`mira/channels/feishu/`](../mira/channels/feishu/) as a multi-instance example.
...
```json
{
  "description": "Use mira from Example chats.",
...
- The locale registry discovers only `mira/channels/*/webui/locales/*.json` and must not import the UI registry, React, or TSX.
...
Put channel-specific Python tests in `mira/channels/<channel>/tests/`. Keep only shared registry, manager, base-class, and cross-channel contract tests in `tests/channels/`. Release builds exclude package-local tests while the repository test configuration discovers both trees.

For a focused channel change, run the smallest relevant set:

```bash
uv run pytest mira/channels/<channel>/tests -q
...
```python
from mira.channels.contracts import ChannelFieldSpec, ChannelSetupSpec, SetupRequirement
from mira.channels.plugin import ChannelPlugin
...
from mira.channels.contracts import ChannelInstanceSpec, ChannelManagementSpec
...
Return a concrete iterable or generator from the adapter's `instance_specs()`; mira materializes and validates it before constructing any runtime. Raise an exception for malformed persisted data rather than silently changing instance identity. Keep network-backed metadata refresh behind the runtime's `refresh_feature_metadata()` so feature GET requests remain dependency-free and read-only.
...
Besides normal assistant text, mira can emit low-emphasis trace blocks. These are intended for UI affordances like status rows, collapsible "used tools" groups, or reasoning/thinking blocks. Platforms that do not have a good place for them can ignore them safely.
...
```python
from mira.bus.outbound_events import ProgressEvent
...
Channel runtimes use Pydantic config models by subclassing `Base` from `mira.config.schema`.

### Pattern

1. Define a Pydantic model inheriting from `mira.config.schema.Base`:
...
`mira onboard` reads the descriptor without importing the runtime. Put writable defaults in `ChannelSetupSpec`:
...
| What | Format | Example |
|------|--------|---------|
| Package directory | `mira/channels/{name}` | `mira/channels/webhook` |
| Manifest name | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Runtime import | `mira.channels.{name}.runtime` | `mira.channels.webhook.runtime` |
