# mira Python SDK: Run an AI Agent from Python

Use mira as a Python library. The SDK gives you the same agent runtime used
...
- mira SDK runs an agent around a model.
...
Install and configure mira first. If you have not done that yet, follow the
...
`~/.mira/workspace/`. Provider, model, tools, memory, and session behavior
...
`mira status` should show the config path, workspace path, active model or
...
from mira.runtime import mira


async def main() -> None:
    async with mira.from_config() as bot:
...
```python
from mira import STREAM_EVENT_TEXT_DELTA
...
Save this as `sdk_demo.py` after `mira agent -m "Hello!"` works:
...
async def main() -> None:
    prompt = " ".join(sys.argv[1:]) or "Explain what mira is in one paragraph."
    ...
    async with mira.from_config() as bot:
        ...
        async for event in bot.stream(prompt, session_key=session_key):
            if event.type == STREAM_EVENT_TEXT_DELTA:
                ...
                raise RuntimeError(event.error or "mira run failed")
...
workspace: /Users/alice/.mira/workspace
...
This script shows the usual production shape: create one `mira`, choose a
...
| Concept | Meaning |
|---------|---------|
| `mira` | The SDK object that owns one configured agent runtime. |
...
| Memory | Long-term memory files managed by mira. |
...
mira has two programming surfaces:

| Use | Choose | Why |
|-----|--------|-----|
| Python code running in the same process as mira | Python SDK | Direct access to `RunResult`, sessions, memory, runtime helpers, hooks, and stream events. |
...
process isolation, or need to call mira from a non-Python service.
...
async with mira.from_config(workspace="/my/project") as bot:
    result = await bot.run("Explain the project structure")
```

Use a custom config when you run multiple mira instances or test an isolated
setup:

```python
async with mira.from_config(
...
The config controls what mira may use. The workspace is where mira keeps
...
```python
bot = mira.from_config(model="openai/gpt-4.1")
```
...
```python
bot = mira.from_config(model_preset="fast")
...
provider and model work with `mira agent -m "Hello!"` first.
...
become mira session history. Ingesting a transcript does not call the model,
...
metrics, tracing, or output post-processing without modifying mira internals:

```python
from mira.agent import AgentHook, AgentHookContext
...
### `mira.from_config(config_path=None, *, workspace=None, model=None, model_preset=None)`
...
```python
from mira.agent import AgentHook
