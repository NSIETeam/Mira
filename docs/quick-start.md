...
| Path | Purpose |
|---|---|
| `~/.mira/config.json` | Provider, model, WebUI, channel, tool, and runtime settings |
| `~/.mira/workspace/` | Sessions, memory, skills, automations, and generated files |
...
If the installer-started WebUI is no longer running, run `mira webui` again. Leave that terminal open; the first-run WebUI is bound to localhost, so other devices on your network cannot reach it.
...
Any normal assistant answer is success. It proves that mira can load the config, reach the selected model, use the workspace, and serve the browser UI.
...
```bash
mira gateway --background
mira gateway status
```

Use `mira gateway logs`, `restart`, and `stop` to manage that background gateway.
...
If the package is installed but the shell cannot find `mira`, use the runner that owns the installation. The recommended installer prints the exact command to reuse. Common forms are:

```bash
uv tool run --from mira mira --version
pipx run --spec mira mira --version
~/.mira/venv/bin/python -m mira --version
```

On Windows, the managed-environment form is `& "$HOME\.mira\venv\Scripts\python.exe" -m mira --version`. Replace `--version` with `webui`, `onboard --wizard`, or any other arguments you need. Use plain `python -m mira` only when that Python executable belongs to the environment where mira was installed.

## Manual Configuration Fallback

Use this only when the wizard is unavailable or you intentionally manage JSON. First run `mira onboard`, then merge a provider and a named model preset into `~/.mira/config.json`.
...
Then check `mira --version`. Run `mira onboard --refresh` when you want to add newly introduced default fields while preserving existing settings.
...
| Symptom | First check |
|---|---|
| `mira: command not found` | Reuse the installer command or method-specific runner described under [Other Install Methods](#other-install-methods) |
...
| WebUI works but a chat app does not | Check **Settings → Channels**, then run `mira channels status` |
