...
echo "Core runtime"
echo "------------"
core_agent=$(count_top_level_py_lines "mira/agent")
core_bus=$(count_top_level_py_lines "mira/bus")
core_config=$(count_top_level_py_lines "mira/config")
core_cron=$(count_top_level_py_lines "mira/cron")
core_session=$(count_top_level_py_lines "mira/session")
...
echo "----------------"
extra_tools=$(count_recursive_py_lines "mira/agent/tools")
extra_skills=$(count_skill_lines "mira/skills")
extra_api=$(count_recursive_py_lines "mira/api")
extra_cli=$(count_recursive_py_lines "mira/cli")
extra_channels=$(count_recursive_py_lines "mira/channels")
extra_utils=$(count_recursive_py_lines "mira/utils")
...
echo "Notes"
echo "-----"
echo "  - agent/ only counts top-level Python files under mira/agent"
echo "  - tools/ is counted separately from mira/agent/tools"
echo "  - skills/ counts .md, .py, and .sh files"
echo "  - not included here: command/, providers/, security/, templates/, mira.py, root files"
