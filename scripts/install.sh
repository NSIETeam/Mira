...
install_failure_hint() {
  printf '%s\n' "Error: could not install mira from $install_source." >&2
  ...
  printf '  %s\n' "$python_bin -m venv ~/.mira/venv" >&2
  printf '  %s\n' "~/.mira/venv/bin/python -m pip install --upgrade $install_target" >&2
  printf '%s\n' "Then start setup with:" >&2
  printf '  %s\n' "mira onboard --wizard" >&2
  exit 1
}
...
run_mira() {
  case "$mira_runner" in
    uv)
      uv tool run --from "$install_target" mira "$@"
      ;;
    pipx)
      pipx run --spec "$install_target" mira "$@"
      ;;
    python)
      "$mira_python" -m mira "$@"
      ;;
...
is_fresh_mira_install() {
  [ -n "${HOME:-}" ] || return 1
  [ ! -e "$HOME/.mira/config.json" ]
}
...
install_with_uv() {
  info "Installing or upgrading mira from $install_source with uv tool..."
  uv tool install --python "$python_bin" --force --upgrade "$install_target" || return 1
  mira_runner="uv"
}

install_with_pipx() {
  info "Installing or upgrading mira from $install_source with pipx..."
  pipx install --python "$python_bin" --force "$install_target" || return 1
  mira_runner="pipx"
}

write_managed_wrapper() {
  bin_dir="${mira_BIN_DIR:-$HOME/.local/bin}"
  wrapper="$bin_dir/mira"
  mkdir -p "$bin_dir" || return 0

  if [ -e "$wrapper" ] && ! grep -q "Generated by mira installer" "$wrapper" 2>/dev/null; then
  ...
  cat > "$wrapper" <<EOF
#!/bin/sh
# Generated by mira installer.
exec "$mira_python" -m mira "\$@"
EOF
  chmod +x "$wrapper" || return 0

  if ! command -v mira >/dev/null 2>&1; then
    info "Installed a mira launcher at $wrapper."
    info "Add $bin_dir to PATH to run mira directly."
  fi
}
...
install_with_managed_venv() {
  ...
  venv_dir="${mira_VENV:-$HOME/.mira/venv}"
  ...
  "$venv_python" - <<'PY' >/dev/null 2>&1 || fail "The managed venv uses Python older than 3.11. Remove it or set mira_VENV to a new path."
import sys
raise SystemExit(0 if sys.version_info >= (3, 11) else 1)
PY

  info "Installing or upgrading mira from $install_source in $venv_dir..."
  ensure_pip "$venv_python" || return 1
  "$venv_python" -m pip install --upgrade "$install_target" || return 1

  mira_runner="python"
  mira_python="$venv_python"
  write_managed_wrapper
}
...
if [ -n "$python_bin" ]; then
  ...
  "$python_bin" - <<'PY' >/dev/null 2>&1 || fail "mira requires Python 3.11 or newer"
import sys
raise SystemExit(0 if sys.version_info >= (3, 11) else 1)
PY
else
...
if [ "$dry_run" = "1" ]; then
  info "Dry run: would install or upgrade mira from $install_source."
  if python_is_virtual_env; then
    ...
    info "Dry run: would run mira as: $python_bin -m mira"
  elif command -v uv >/dev/null 2>&1; then
    info "Dry run: would run: uv tool install --python $python_bin --force --upgrade $install_target"
    info "Dry run: would run mira as: uv tool run --from $install_target mira"
  elif command -v pipx >/dev/null 2>&1; then
    info "Dry run: would run: pipx install --python $python_bin --force $install_target"
    info "Dry run: would run mira as: pipx run --spec $install_target mira"
  else
    venv_dir="${mira_VENV:-$HOME/.mira/venv}"
    ...
    info "Dry run: would run mira as: $venv_dir/bin/python -m mira"
  fi
  if [ "${mira_SKIP_WIZARD:-}" = "1" ]; then
    info "Dry run: would skip automatic setup because mira_SKIP_WIZARD=1."
  elif is_fresh_mira_install && has_browser_session; then
...
if [ "${mira_SKIP_WIZARD:-}" = "1" ]; then
  info "Skipping automatic setup because mira_SKIP_WIZARD=1."
  info "Run this later: $(mira_try_command) webui"
  exit 0
fi

if is_fresh_mira_install && has_browser_session; then
  if run_mira webui --help >/dev/null 2>&1; then
    info "Starting mira WebUI..."
    info "Configure your first provider and model in Settings > Models."
    info "Run this later: $(mira_try_command) webui"
    run_mira webui --yes
    exit 0
  fi
  info "The installed release does not support mira webui yet."
...
if [ -t 0 ]; then
  ...
  info "Run this later: $(mira_try_command) onboard --wizard"
fi

info "Done. Try: $(mira_try_command) agent -m \"Hello!\""
