...
from mira.agent.tools.web import WebFetchTool, _validate_url
...
def _patched_web_fetch():
    with (
        patch("mira.security.network.socket.getaddrinfo", _fake_resolve_public),
        patch("mira.agent.tools.web.httpx.AsyncClient", FakeClient),
        patch("mira.agent.tools.web._pinned_dns_transport", lambda: object()),
    ):
        yield
