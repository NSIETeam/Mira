...
import mira.channels._setup as channel_setup_module
import mira.channels.registry as registry_module
from mira.channels._setup import channel_setup_spec
from mira.channels.plugin import ChannelPlugin, load_channel_package
from mira.channels.registry import channel_default_enabled, discover_plugins
...
def test_channel_manifests_only_import_contract_modules() -> None:
    channel_dir = Path(channel_setup_module.__file__).parent
    allowed_imports = {
        "mira.channels._manifest",
        "mira.channels.contracts",
        "mira.channels.plugin",
    }
    ...
    for name in EXPECTED_CHANNELS:
        ...
        allowed_channel_imports = {
            ...
            if module.startswith(f"mira.channels.{name}.")
...
def test_feishu_package_manifest_owns_runtime_and_webui_metadata() -> None:
    ...
    assert plugin is not None
    assert plugin.runtime == "mira.channels.feishu.runtime:FeishuChannel"
    assert plugin.dependencies == ("lark-oapi>=1.5.0,<2.0.0",)
    assert plugin.connector == "mira.channels.feishu.connect:FeishuConnectStore"
...
def test_weixin_package_manifest_owns_runtime_and_webui_metadata() -> None:
    ...
    assert plugin is not None
    assert plugin.runtime == "mira.channels.weixin.runtime:WeixinChannel"
    assert plugin.dependencies == ("qrcode[pil]>=8.0", "pycryptodome>=3.20.0")
    assert plugin.connector == "mira.channels.weixin.connect:WeixinConnectStore"
    assert plugin.webui == "webui/index.tsx"
...
for name in {sorted(EXPECTED_CHANNELS)!r}:
    plugin = load_channel_package(name)
    assert plugin is not None
    assert f"mira.channels.{{name}}.runtime" not in sys.modules
"""
