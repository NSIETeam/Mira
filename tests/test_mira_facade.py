...
from mira.utils.llm_runtime import runtime_from_provider_snapshot
...
def test_from_config_accepts_default_model_preset(tmp_path):
    ...
    bot = mira.from_config(config_path, workspace=tmp_path, model_preset="fast")
...
def test_from_config_default_path():
    ...
    with patch("mira.config.loader.load_config") as mock_load, \
         patch("mira.providers.factory.make_provider") as mock_prov:
...
async def test_run_with_hooks(tmp_path):
    from mira.agent.hook import AgentHook, AgentHookContext, SDKCaptureHook
    from mira.bus.events import OutboundMessage
...
def test_import_from_top_level():
    ...
    assert mira.mira is mira
    assert mira.RunResult is RunResult
    assert mira.RunStream is RunStream
    assert mira.SessionInfo is SessionInfo
    assert mira.SessionSnapshot is SessionSnapshot
    assert mira.StreamEvent is StreamEvent
    assert mira.StreamEventType is StreamEventType
    assert mira.STREAM_EVENT_TEXT_DELTA == STREAM_EVENT_TEXT_DELTA
    assert mira.STREAM_EVENT_RUN_COMPLETED == STREAM_EVENT_RUN_COMPLETED
    assert mira.STREAM_EVENT_TYPES == STREAM_EVENT_TYPES
...
async def test_run_populates_observability_fields(tmp_path):
    from mira.agent.hook import AgentRunHookContext
    from mira.bus.events import OutboundMessage
...
async def test_sdk_capture_prefers_run_level_snapshot():
    from mira.agent.hook import AgentHookContext, AgentRunHookContext, SDKCaptureHook
    from mira.providers.base import ToolCallRequest
