...
class TestSaveFsync:
    ...
    def test_save_ignores_unsupported_directory_fsync(
        ...
        directory_fd = 987654
        with (
            patch("mira.session.manager.os.open", return_value=directory_fd) as open_dir,
            patch(
                "mira.session.manager.os.fsync",
                side_effect=[None, OSError(errno.EINVAL, "Invalid argument")],
            ),
            patch("mira.session.manager.os.close") as close_dir,
        ):
            manager.save(session, fsync=True)
    ...
    def test_save_propagates_other_directory_fsync_errors(
        ...
        directory_fd = 987654
        with (
            patch("mira.session.manager.os.open", return_value=directory_fd),
            patch(
                "mira.session.manager.os.fsync",
