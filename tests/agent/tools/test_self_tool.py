...
class TestSubagentHookStatus:
    ...
    async def test_after_iteration_updates_status(self):
        ...
        from mira.agent.subagent import SubagentStatus, _SubagentHook
    ...
    async def test_after_iteration_no_status_is_noop(self):
        ...
        from mira.agent.hook import AgentHookContext
        from mira.agent.subagent import _SubagentHook
...
class TestRuntimeVarsInspectFallback:
    ...
    async def test_inspect_runtime_var_string(self):
        tool = _make_tool()
        await tool.execute(action="set", key="current_project", value="mira")
...
class TestRequestContext:
    ...
    def test_audit_reads_bound_session(self):
        ...
        with patch("mira.agent.tools.self.logger.info") as info:
            with request_context(ctx):
