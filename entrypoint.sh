...
# re-exec as mira. Fail closed: if the privilege drop cannot be performed,
# exit rather than run the agent as root.
if [ "$(id -u)" = "0" ]; then
    chown -R mira:mira "$dir" 2>/dev/null || echo "[entrypoint] warning: chown $dir failed"
    if setpriv --reuid=mira --regid=mira --init-groups true 2>/dev/null; then
        echo "[entrypoint] dropping privileges to mira via setpriv"
        exec setpriv --reuid=mira --regid=mira --init-groups mira "$@"
    fi
